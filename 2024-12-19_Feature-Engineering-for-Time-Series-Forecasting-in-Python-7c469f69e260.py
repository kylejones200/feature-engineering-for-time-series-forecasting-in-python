...
# Original values
axes[0].plot(
    train.index, train["values"], label="Train", linewidth=1.2, color="#2c3e50"
)
...
# Scaled values
axes[1].plot(
    train.index, train["scaled"], label="Train (Scaled)", linewidth=1.2, color="#2c3e50"
)
axes[1].plot(
    test.index, test["scaled"], label="Test (Scaled)", linewidth=1.2, color="#e74c3c"
)
...
ax.set_title(
    f"Exponential Smoothing Forecast (MAPE: {mape * 100:.2f}%)", fontweight="bold"
)
ax.set_xlabel("Date")
...
def main():
    ...
    ax1.plot(
        test_indices, y_pred_test, label="Predicted", linewidth=2, alpha=0.7, linestyle="--"
    )
