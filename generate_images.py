...
total_use = df_long[df_long['MSN'].str.contains('TOT', na=False)].copy()
total_use = total_use.groupby('Year')['Value'].sum().reset_index()
total_use = total_use[total_use['Value'].notna() & (total_use['Value'] > 0)]

df_base = total_use.set_index('Year').copy()
df_base['value'] = df_base['Value']
df_base = df_base[['value']]
